...
- 03260a5: Add concurrency to Notion document push and block deletion for dramatically faster sync of large workspaces.
...
- 54e031a: Add Library select property to Notion document database schema for tagging documents with their library path (e.g., 'documentation/api-reference'). The property is auto-created if missing and supports both push (create/update) and pull paths.
- eb08c40: Add Notion REST API client with types, pagination, rate limit retry, and typed error handling for document sync.
- 66a831b: Add progress bar to `link-all` and `push --all` commands. The progress bar displays on stderr in interactive TTY sessions and is suppressed in `--json`, `--quiet`, and non-TTY (piped/CI) modes.
...
- a65a4dd: Add large element set warning for link-all and push --all: when operating on >100 elements, a warning is shown on stderr informing the user the operation may take significant time. Warnings are suppressed in --json, --quiet, and --dry-run modes.
...
- 1033e1d: Resolve and pass libraryPath during document push. Documents belonging to libraries now include their library path in ExternalDocumentInput, enabling providers to organize documents by library hierarchy. Library paths are batch-resolved before the push loop to avoid N+1 queries.
- 6a769ce: Skip documents without a library by default in `link-all` and `push` commands. Add `--no-library` flag to opt-in to including library-less documents. Add `includeNoLibrary` option to `SyncOptions` for programmatic control.
...
- e50386c: Enforce Notion rich_text array length limit (max 100 elements). When inline-heavy markdown produces more than 100 rich_text elements, blocks are split into multiple blocks. Adjacent plain text elements with identical formatting are merged to reduce array count.
...
- e262b2e: Remove vestigial @stoneforge/smithy peerDependency that created a cyclic dependency breaking turbo build
- 4c3c57f: Retry transient Notion API errors (502/503/504) with exponential backoff. Also update sync-engine isRetryableError() to detect 502/504 patterns.
- 7ccf4e1: Skip archived blocks during Notion page content replacement to avoid "Can't edit block that is archived" errors.
...
- a261b37: Validate URLs before creating Notion link blocks to prevent "Invalid URL for link" API errors; invalid URLs (relative paths, fragments, element IDs, malformed) now render as plain text
